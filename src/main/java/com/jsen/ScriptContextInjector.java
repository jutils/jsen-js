 ...
 * script engine factory via {@link ScriptEngineManager}.
 * 
 * @author Radim Loskot
 * @version 0.9
 * @since 0.9 - 21.4.2014
...
public abstract class ScriptContextInjector extends ScriptContextInject {
	...
	protected final static ScriptEngineManager manager = ScriptEngineManager.getInstance();
	...
	public void registerScriptContextInject() {
		if (scriptEngineNames != ALL_SCRIPT_ENGINE_FACTORIES) {
			for (String scriptEngineName : scriptEngineNames) {
				Set<AbstractScriptEngineFactory> factories = manager.getMimeContentFactories(scriptEngineName); 
				registerForScriptEngineFactories(factories);
			}
		} else {
			Set<AbstractScriptEngineFactory> factories = manager.getAllMimeContentFactories(); 
			registerForScriptEngineFactories(factories);
		}
	...
	private void registerForScriptEngineFactories(Set<AbstractScriptEngineFactory> factories) {
		for (AbstractScriptEngineFactory factory : factories) {
			factory.registerScriptContextsInject(this);
		}
	}
	
	private void unregisterForScriptEngineFactories(Set<AbstractScriptEngineFactory> factories) {
		for (AbstractScriptEngineFactory factory : factories) {
